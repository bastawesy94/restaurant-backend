...
module.exports = class extends CompanyRepo{
   constructor({db}){
    ...
    getAllCompaniesByCategoryServiceId(categoryServiceId){
            return this.db.sequelize.query('SELECT company.id , company.name , AVG(review.stars) as rating FROM company LEFT JOIN review ON review.company_id = company.id WHERE company.category_service_id= $1 GROUP BY company.id',{
    ...
    getAllCompaniesWithRatesByCategoryId(categoryId){
        return this.db.sequelize.query('SELECT company.id , company.name , AVG(review.stars) as rating FROM company LEFT JOIN review ON review.company_id = company.id INNER JOIN category_service ON company.category_Service_id = category_service.id WHERE category_service.category_id=$1 GROUP BY company.id',{
            bind: [categoryId],
            model: this.db.Company,
            type: QueryTypes.SELECT
        })
